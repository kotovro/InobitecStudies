...
struct PpmResult ppm_read(FILE* f) {
    struct PpmResult result = {PRE_OK, "", {0, 0, 0, NULL}};
    int line_num = 1;
    ...
    if (c != '3') {
        ...
        if (c >= ' ' && c <= '~')
            ...
            snprintf(result.diagnostic, sizeof(result.diagnostic),
                     "строка %d: ожидалось 'P3', код 0x%02x после 'P'", line_num,
                     (unsigned char)c);
        return result;
    }
    ...
    if (w_val <= 0 || w_val > 0x7FFFFFFF) {
        result.error = PRE_BAD_NUMBER;
        snprintf(result.diagnostic, sizeof(result.diagnostic),
                 "строка %d: ширина должна быть положительным числом; получено: %lld",
                 line_num, w_val);
        return result;
    }
    ...
    if (h_val <= 0 || h_val > 0x7FFFFFFF) {
        result.error = PRE_BAD_NUMBER;
        snprintf(result.diagnostic, sizeof(result.diagnostic),
                 "строка %d: высота должна быть положительным числом; получено: %lld",
                 line_num, h_val);
        return result;
    }
    ...
    if (pixel_error) {
        if (result.error == PRE_BAD_NUMBER && pixel_count < total_pixels && feof(f)) {
            ...
            snprintf(result.diagnostic, sizeof(result.diagnostic),
                     "строка %d: получено только %lld пикселей (ожидалось %lld)",
