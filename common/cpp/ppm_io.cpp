...
PpmResult ppm_read(std::istream& is) {
...

    {
        ...
        img.max_val = static_cast<uint16_t>(m);
        img.pixels.reserve(static_cast<std::size_t>(w) * static_cast<std::size_t>(h));
    }

    // ---- 3. Pixel data ----
    phase = Phase::kData;
    long long total_pixels = static_cast<long long>(img.width) * img.height;
    ...
    for (long long i = 0; i < total_pixels; ++i) {
        ...
        if (is.eof())
            return err(PpmReadError::kTooFewPixels,
                       std::format("строка {}: получено только {} пикселей (ожидалось {})",
                                   line_num, img.pixels.size(), total_pixels));
        ...
        if (r < 0 || r > img.max_val || g < 0 || g > img.max_val || b < 0 || b > img.max_val)
            return err(PpmReadError::kChannelRange,
                       std::format("строка {}: значение канала должно быть в [0; {}]; "
                                   "получено: {} {} {}",
                                   line_num, img.max_val, r, g, b));

        img.pixels.push_back(Pixel{static_cast<uint8_t>(r), static_cast<uint8_t>(g), static_cast<uint8_t>(b)});
    }
    ...
    if (!is.eof()) {
        ...
        if (!is.eof())
            return err(PpmReadError::kTooManyPixels,
                       std::format("строка {}: лишние данные после {} пикселей",
                                   line_num, img.pixels.size()));
    }
...
PpmWriter::PpmWriter(std::ostream& os, int32_t width, int32_t height)
    : _os(os), _width(width) {
    std::println(_os, "P3");
...
void PpmWriter::put(uint8_t r, uint8_t g, uint8_t b) {
    if (_col == 0) {
        std::print(_os, "{:3d} {:3d} {:3d}", static_cast<int>(r),
